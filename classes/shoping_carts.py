...
class ShoppingCart:
    ...
    def get_details(self, customer, admin):
        ...
        а также информацию о покупателе и администраторе.
        """
        details = f"Корзина покупок для {customer.username}:\n"
        for item in self.items:
            details += f"{item['Продукт'].get_details()}, Количество: {item['количество']}\n"
        details += f"Общее: {self.get_total()} руб\n"
        details += f"Регистрация покупок: пользователь {admin.username}\n"
        return details